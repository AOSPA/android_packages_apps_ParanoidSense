...
import co.aospa.sense.camera.CameraFaceAuthController
import co.aospa.sense.camera.CameraFaceEnrollController
...
class SenseService : Service() {
    ...
    private var mCameraAuthController: CameraFaceAuthController? = null
    private var mCameraEnrollController: CameraFaceEnrollController? = null
    ...
    private val mCameraAuthControllerCallback: CameraFaceAuthController.ServiceCallback =
        object : CameraFaceAuthController.ServiceCallback {
    ...
    private val mCameraEnrollServiceCallback: CameraFaceEnrollController.CameraCallback = object :
        CameraFaceEnrollController.CameraCallback {
        ...
        override fun enroll(cryptoToken: ByteArray?, timeout: Int, disabledFeatures: IntArray?) {
            ...
            if (Util.isFaceUnlockDisabledByDPM(this@SenseService) || mChallenge == 0L || cryptoToken == null) {
                ...
                mWorkHandler!!.post {
                    ...
                    synchronized(this) {
                        if (mCameraEnrollController == null) {
                            mCameraEnrollController = CameraFaceEnrollController.instance
                        }
        ...
        override fun authenticate(operationId: Long) {
            ...
            if (!Util.isFaceUnlockAvailable(this@SenseService) ||
                ...
                mWorkHandler!!.post {
                    ...
                    synchronized(this) {
                        if (mCameraAuthController == null) {
                            mCameraAuthController = CameraFaceAuthController(
                                this@SenseService,
