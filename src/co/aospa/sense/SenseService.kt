...
class SenseService : Service() {
    ...
    private val lockoutTypeLock = Any()
    private val authenticationErrorCountLock = Any()
    ...
    private val mReceiver: BroadcastReceiver = object : BroadcastReceiver() {
        override fun onReceive(context: Context, intent: Intent) {
            ...
            when (action) {
                ALARM_TIMEOUT_FREEZED -> synchronized(lockoutTypeLock) {
                    mLockoutType = LOCKOUT_TYPE_IDLE
                }
                ALARM_FAIL_TIMEOUT_LOCKOUT -> {
                    cancelLockoutTimer()
                    synchronized(lockoutTypeLock) { mLockoutType = LOCKOUT_TYPE_DISABLED }
                    synchronized(authenticationErrorCountLock) { mAuthenticationErrorCount = 0 }
                }
    ...
    private fun increaseAndCheckLockout() {
        ...
        synchronized(authenticationErrorCountLock) {
